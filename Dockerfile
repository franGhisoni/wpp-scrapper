...
# Ensure logger.ts exists with TypeScript types to match usage
...
# Create duplicate of logger.ts as Logger.ts to handle case sensitivity issues
RUN cp ./src/utils/logger.ts ./src/utils/Logger.ts

# Update tsconfig.prod.json to disable case sensitivity checking
RUN if grep -q "removeComments" tsconfig.prod.json; then \
    sed -i 's/"removeComments": true/"removeComments": true,\n    "forceConsistentCasingInFileNames": false/' tsconfig.prod.json; \
    else \
    sed -i '/"compilerOptions": {/a \    "forceConsistentCasingInFileNames": false,' tsconfig.prod.json; \
    fi
